...
window.createPin = (function () {
  ...
  for (var i = 1; i < OFFERS_COUNT; i++) {
    ...
    buttonPin.style.left = window.getData()[i].location.x + pinShiftX + 'px';
    buttonPin.style.top = window.getData()[i].location.y + pinShiftY + 'px';

    var imgPin = document.createElement('img');
    imgPin.src = window.getData()[i].author.avatar;
    imgPin.width = 40;
    imgPin.height = 40;
